...
server <- function(input, output) {
  ...
  output$resultC <- renderText({
    c_prop <- prod(c_vals()) * input$covidPrev
    f_prop <- prod(f_vals()) * input$fluPrev
    n_prop <- prod(s_vals()) * (1-input$fluPrev-input$covidPrev)
  ...
  output$most_likely <- renderText({
    final_prob <- ""
    if(max(c(resultC, resultF, resultN)) == resultC){
      final_prob <- "COVID-19"
    }
    if(max(c(resultC, resultF, resultN)) == resultF){
      final_prob <- "Flu"
    }
    if(max(c(resultC, resultF, resultN)) == resultN){
      final_prob <- "Neither"
    }
  })
